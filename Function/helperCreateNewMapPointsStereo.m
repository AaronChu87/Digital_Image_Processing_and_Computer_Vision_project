...
currLocations   = currPoints.Location; % 2D location
currScales      = currPoints.Scale;    % ORB feature scale
...
for i = numel(KcIDs):-1:1
    ...
    % Pose and 3d points in each connected keyframe.
    ...
    % Only use unmatched feature points (not in frame's view)
    uIndices1   = setdiff(uint32(1:size(kfFeatures,1))',  kfIndex2d); % unmatched feature points index
    ...
    indexPairs  = matchFeatures(uFeatures1, uFeatures2,...
        'Unique', true, 'MaxRatio', 0.7, 'MatchThreshold', 4);
    ...
    % Created from stereo
    [~, ia, ib] = intersect(unmatchedFeatureIdx, uIndices2(indexPairs(:, 2))); % find matched feature from untracked features
